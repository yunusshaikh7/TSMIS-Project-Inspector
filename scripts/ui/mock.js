...
function makeMockApi() {
  ...
  const rows = [
    { name: files[0], folder: "…\\GIS_Projects\\D07", status: "ok", status_text: "OK", versions: ["KELLY.D07_I5_2026"], layers: 14, message: "" },
    { name: files[1], folder: "…\\GIS_Projects\\QA", status: "ok", status_text: "OK", versions: ["sde.DEFAULT", "KELLY.QA_check"], layers: 22, message: "" },
    { name: files[2], folder: "…\\GIS_Projects", status: "no_versions", status_text: "No version found", versions: [], layers: 3, message: "data connections found, but none names a version" },
    { name: files[3], folder: "…\\GIS_Projects\\archive", status: "no_connections", status_text: "No data connections", versions: [], layers: 0, message: "no data connections found (6 files inside, 4 JSON)" },
    { name: files[4], folder: "…\\GIS_Projects", status: "error", status_text: "Error", versions: [], layers: 0, message: "not a valid project file (not a zip archive)" },
  ];
  let cancelled = false;
  return {
    ...
    start_scan: async (root) => {
      ...
      files.forEach((f, i) => setTimeout(() => {
        if (cancelled) return;
        const done = i;
        state.scan = { done, total: files.length, current: root + "\\" + f };
        d([{ t: "progress", p: state.scan }]);
        const r = rows[i];
        setTimeout(() => d([{ t: "log", text: r.status === "ok" ? `  ${f}: ${r.versions.join(", ")}` : `  ${f}: ${r.status_text} — ${r.message}` }]), 350);
      }, 900 + i * 700));
      setTimeout(() => {
        if (cancelled) return;
        ...
        state.last_scan = { ok: true, cancelled: false, root, rows,
          ...
          workbook: settings.meta.output_root + "\\2026-09-04 14-30-12\\branch_versions.xlsx",
          run_dir: settings.meta.output_root + "\\2026-09-04 14-30-12" };
        d([{ t: "log", text: "" }, { t: "log", text: "Read 5 file(s) under " + root + ": 2 with a version, 1 without a version, 1 without data connections, 1 error(s)." },
           { t: "log", text: "Versions found: sde.DEFAULT (1 project), KELLY.D07_I5_2026 (1 project), KELLY.QA_check (1 project)" },
           { t: "log", text: "Workbook saved: " + state.last_scan.workbook }, { t: "run_ended" }]);
        push();
